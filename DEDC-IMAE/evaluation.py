...
def eva(y_true, y_pred, epoch=0,file_out=0):
    # acc, f1 = cluster_acc(y_true, y_pred)
    acc = cluster_acc1(y_true, y_pred)[0]*100
    nmi = cluster_nmi1(y_true, y_pred)*100
    # nmi = nmi_score(y_true, y_pred, average_method='arithmetic')
    ari = ari_score(y_true, y_pred)*100
    # print(epoch, ':acc {:.4f}'.format(acc), ', nmi {:.4f}'.format(nmi), ', ari {:.4f}'.format(ari),
    #         ', f1 {:.4f}'.format(f1))
    # print(acc,nmi)
    # print(epoch, ':acc {:.4f}'.format(acc), ', nmi {:.4f}'.format(nmi),', ari {:.4f}'.format(ari))
    print('eopch:{},acc:{:.4f}, nmi:{:.4f}, ari:{:.4f}'.format(epoch,acc,nmi,ari))
    return acc

def eva2(y_true, y_pred,file_out=0):
    # acc, f1 = cluster_acc(y_true, y_pred)
    acc = cluster_acc1(y_true, y_pred)[0]*100
    nmi = cluster_nmi1(y_true, y_pred)*100
    # nmi = nmi_score(y_true, y_pred, average_method='arithmetic')
    ...
    #         ', f1 {:.4f}'.format(f1))
    # print(acc,nmi)
    print('the time predict result:,acc:{:.4f}, nmi:{:.4f}, ari:{:.4f}'.format(acc,nmi,ari))

def eva3(y_true, y_pred,epoch=0):
    # acc, f1 = cluster_acc(y_true, y_pred)
    acc = cluster_acc1(y_true, y_pred)[0]*100
    nmi = cluster_nmi1(y_true, y_pred)*100
    # nmi = nmi_score(y_true, y_pred, average_method='arithmetic')
